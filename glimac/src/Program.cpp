...
Program loadProgram(const FilePath& vsFile, const FilePath& fsFile) {
	...
	if(!vs.compile()) {
        std::cout<<"Compilation error for vertex shader (from file " + std::string(vsFile) + "): "<< vs.getInfoLog()<<std::endl;
		throw std::runtime_error("Compilation error for vertex shader (from file " + std::string(vsFile) + "): " + vs.getInfoLog());
	}

	if(!fs.compile()) {
        std::cout << "Compilation error for vertex shader (from file " + std::string(fsFile) + "): " << fs.getInfoLog() << std::endl;
